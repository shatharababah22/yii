...
<div class="card">
  ...
  <thead class="thead-light">
    <tr>
      ...
      <th><?= Yii::t('app', 'Depart Country Code') ?></th>
      <th><?= Yii::t('app', 'Departure Date') ?></th>
      <th><?= Yii::t('app', 'Going To') ?></th>
      <th><?= Yii::t('app', 'Arrival Country Code') ?></th>
      <th><?= Yii::t('app', 'Return Date') ?></th>
      <th><?= Yii::t('app', 'Proposal State') ?></th>
      <th><?= Yii::t('app', 'Itinerary ID') ?></th>
      <th><?= Yii::t('app', 'PNR') ?></th>
      <th><?= Yii::t('app', 'Policy No') ?></th>
      <th><?= Yii::t('app', 'Policy Purchased Date Time') ?></th>
      <th><?= Yii::t('app', 'Policy URL Link') ?></th>
      <th><?= Yii::t('app', 'Status') ?></th>
      <th><?= Yii::t('app', 'Status Description') ?></th>
  ...
  <?php if ($dataProvider->models) : ?>
    <?php foreach ($dataProvider->models as $policy) : ?>
      <tr>
        ...
        <td><?= Html::encode($policy->DepartCountryCode) ?></td>
        <td><?= Html::encode(Yii::$app->formatter->asDate($policy->departure_date)) ?></td>
        <td><?= Html::encode($policy->going_to) ?></td>
        <td><?= Html::encode($policy->ArrivalCountryCode) ?></td>
        <td><?= Html::encode(Yii::$app->formatter->asDate($policy->return_date)) ?></td>
        <td><?= Html::encode($policy->ProposalState) ?></td>
        <td><?= Html::encode($policy->ItineraryID) ?></td>
        <td><?= Html::encode($policy->PNR) ?></td>
        <td><?= Html::encode($policy->PolicyNo) ?></td>
        <td><?= Html::encode($policy->PolicyPurchasedDateTime) ?></td>
        <td><?= Html::encode($policy->PolicyURLLink) ?></td>
        <td><?= Html::encode($policy->status == 0 ? "Inactive" : "Active") ?></td>
        <td><?= Html::encode($policy->status_description) ?></td>
        <td><?= Html::encode($policy->source) ?></td>
        <td><?= Html::encode(Yii::$app->formatter->asCurrency($policy->price)) ?></td>
        
      </tr>
    <?php endforeach; ?>
    <?php else: ?>
            <tr class="odd"><td valign="top" colspan="17" class="dataTables_empty"><div class="text-center p-4">
